...
class FileConverterApp:
		...
		def select_file(self):
				file_path = filedialog.askopenfilename(title="Seleccionar archivo MPL", filetypes=(("Archivos MPL", "*.mpl"),))
				if file_path:
		...
		def convert_file(self):
				if self.file_path and self.file_path.endswith(".mpl"):
						...
						try:
								...
								self.result_text.insert(tk.END, f"Archivo convertido automáticamente: {dzn_file_path}\n")
		...
		def run_model(self):
				...
				if os.path.exists(dzn_file_path):
						try:
								...
								result = subprocess.run(["minizinc", minizinc_model_path, dzn_file_path], capture_output=True, text=True)
